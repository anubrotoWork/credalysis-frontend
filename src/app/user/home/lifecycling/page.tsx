...
export default function LifecyclePage() {
  ...
  const [activeTab, setActiveTab] = useState<'goals' | 'products' | 'lifecycle'>('lifecycle');
  ...
  useEffect(() => {
    ...
    fetch(`http://34.9.145.33:8000/api/user/lifecycle/?email=${email}`)
      .then((res) => res.json())
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex space-x-4 mb-4">
        <button
          className={`px-4 py-2 rounded ${activeTab === 'goals' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
      ...
      {activeTab === 'goals' && (
        <div className="space-y-4">
          {data.goals.map((goal) => (
            <div key={goal.goal_id || goal.goal_name} className="p-4 border rounded shadow">
              <h2 className="text-xl font-semibold">{goal.goal_name}</h2>
              <p>Type: {goal.goal_type}</p>
              <p>Status: {goal.status} ({goal.priority})</p>
              ...
              <p>Required Monthly Contribution: ${goal.required_monthly_contribution.toFixed(2)}</p>
              <p>Actual Monthly Contribution: ${goal.actual_monthly_contribution.toFixed(2)}</p>
      ...
      {activeTab === 'products' && (
        <div className="space-y-4">
          {data.products.map((product) => (
            <div key={product.customer_product_id} className="p-4 border rounded shadow">
              ...
              <p>Payment: ${product.payment_amount.toFixed(2)} {product.payment_frequency || ''}</p>
      ...
      {activeTab === 'lifecycle' && (
        <div className="prose max-w-none">
          <ReactMarkdown remarkPlugins={[remarkGfm]}>{data.lifecycle}</ReactMarkdown>
        </div>
      )}
    </div>
  );
